...
namespace pimoroni {
  ...
  IOExpander::IOExpander(TwoWire *i2c, uint8_t address, uint32_t timeout, bool debug) :
    i2c(i2c),
    address(address),
  ...
  int i2c_write_blocking(TwoWire* i2c, uint8_t address, const uint8_t* src, size_t len, bool nostop)
  {
  ...
  int i2c_read_blocking(TwoWire* i2c, uint8_t address, uint8_t* src, size_t len, bool nostop)
  {
    i2c->requestFrom(address, len);
    i2c->readBytes(src, len);
    return len;
  }
  ...
  uint8_t IOExpander::check_reset() {
    ...
    uint8_t value;
    if(i2c_write_blocking(i2c, address, &user_flash_reg, 1, false) == 0) {
      return 0x00;
    }
    if(i2c_read_blocking(i2c, address, (uint8_t *)&value, sizeof(uint8_t), false) == 0) {
      return 0x00;
    }
...
//  }
  ...
  uint8_t reg_read_uint8(TwoWire* i2c, int8_t address, uint8_t reg_address)
  {
    i2c->beginTransmission(address);
    i2c->write(reg_address);
    i2c->endTransmission();
    i2c->requestFrom(address, 1); // This register is 8 bits = 1 byte long
    delay(2); // Wait for data to be available
    // Read directly into an uint8_t
    return (uint8_t)i2c->read();
  }

  void reg_write_uint8(TwoWire* i2c, int8_t address, uint8_t reg_address, uint8_t value)
  {
    i2c->beginTransmission(address);
    i2c->write(reg_address);
    i2c->write(value);
  ...
  uint16_t IOExpander::get_chip_id() {
      return ((uint16_t)reg_read_uint8(i2c, address, reg::CHIP_ID_H) << 8) | (uint16_t)reg_read_uint8(i2c, address, reg::CHIP_ID_L);
  }

  void IOExpander::set_address(uint8_t address) {
    set_bit(reg::CTRL, 4);
    reg_write_uint8(i2c, this->address, reg::ADDR, address);
  ...
  bool IOExpander::set_pwm_control(uint8_t divider) {
    ...
    if(divider_good) {
      ...
      reg_write_uint8(i2c, address, reg::PWMCON1, pwmdiv2);
    }
  ...
  void IOExpander::set_pwm_period(uint16_t value, bool load, bool wait_for_load) {
    value &= 0xffff;
    reg_write_uint8(i2c, address, reg::PWMPL, (uint8_t)(value & 0xff));
    reg_write_uint8(i2c, address, reg::PWMPH, (uint8_t)(value >> 8));
  ...
  void IOExpander::set_mode(uint8_t pin, uint8_t mode, bool schmitt_trigger, bool invert) {
    ...
    uint8_t pm1 = reg_read_uint8(i2c, address, io_pin.reg_m1);
    uint8_t pm2 = reg_read_uint8(i2c, address, io_pin.reg_m2);
    ...
    reg_write_uint8(i2c, address, io_pin.reg_m1, pm1);
    reg_write_uint8(i2c, address, io_pin.reg_m2, pm2);
    ...
    reg_write_uint8(i2c, address, io_pin.reg_p, (initial_state << 3) | io_pin.pin);
  }
  ...
  int16_t IOExpander::input(uint8_t pin, uint32_t adc_timeout) {
    ...
    if(io_pin.get_mode() == PIN_MODE_ADC) {
      ...
      reg_write_uint8(i2c, address, reg::AINDIDS, 0);
      ...
      uint8_t hi = reg_read_uint8(i2c, address, reg::ADCRH);
      uint8_t lo = reg_read_uint8(i2c, address, reg::ADCRL);
  ...
  void IOExpander::output(uint8_t pin, uint16_t value, bool load, bool wait_for_load) {
    ...
    if(io_pin.get_mode() == PIN_MODE_PWM) {
      ...
      reg_write_uint8(i2c, address, io_pin.reg_pwml, (uint8_t)(value & 0xff));
      reg_write_uint8(i2c, address, io_pin.reg_pwmh, (uint8_t)(value >> 8));
      if(load)
        pwm_load(wait_for_load);
    }
    else {
  ...
  void IOExpander::setup_rotary_encoder(uint8_t channel, uint8_t pin_a, uint8_t pin_b, uint8_t pin_c, bool count_microsteps) {
    ...
    reg_write_uint8(i2c, address, ENC_CFG[channel], pin_a | (pin_b << 4));
    ...
    reg_write_uint8(i2c, address, reg, 0x00);
  }
  ...
  int16_t IOExpander::read_rotary_encoder(uint8_t channel) {
    ...
    uint8_t reg = ENC_COUNT[channel];
    int16_t value = (int16_t)reg_read_uint8(i2c, address, reg);
  ...
  void IOExpander::set_bits(uint8_t reg, uint8_t bits) {
    if(reg == reg::P0 || reg == reg::P1 || reg == reg::P2 || reg == reg::P3) {
        for(uint8_t bit = 0; bit < 8; bit++) {
          if(bits & (1 << bit)) {
            reg_write_uint8(i2c, address, reg, 0b1000 | (bit & 0b111));
            sleep_us(50);
          }
        }
        return;
    ...
    uint8_t value = reg_read_uint8(i2c, address, reg);
    sleep_us(50);
    reg_write_uint8(i2c, address, reg, value | bits);
  }
  ...
  void IOExpander::clr_bits(uint8_t reg, uint8_t bits) {
    if(reg == reg::P0 || reg == reg::P1 || reg == reg::P2 || reg == reg::P3) {
      for(uint8_t bit = 0; bit < 8; bit++) {
        if(bits & (1 << bit)) {
          reg_write_uint8(i2c, address, reg, 0b0000 | (bit & 0b111));
          sleep_us(50);
        }
      }
      return;
